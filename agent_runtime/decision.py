...
Phase 2: Adds DELEGATE action for L3 task creation, and
LLM-assisted classification for ambiguous intents.
"""
...
class Intent(str, Enum):
    ...
    DATA_QUERY = "data_query"       # Look up price/rate/status
    DELEGATION = "delegation"       # Create a task, ask another agent
...
class Action(str, Enum):
    ...
    DELEGATE = "delegate"         # Create and delegate an ARMP Task
...
class DecisionEngine:
    ...
    PATTERNS = {
        ...
        Intent.DATA_QUERY: [
            r"\b(price|cost|rate|status|check|lookup|query)\b",
            r"\b(how much|how many)\b",
        ],
        Intent.DELEGATION: [
            r"\b(assign|delegate|hand off|send to|ask.*agent)\b",
            r"\b(need.*agent|find.*agent|contact.*agent)\b",
        ],
    }
    ...
    API_PATTERNS = {
        ...
        "task": [r"\b(task|job|work item|assignment)\b"],
    }

    def __init__(self, runtime: "Runtime"):
        self.runtime = runtime
        self._use_llm = False  # Phase 1: keyword-only
    ...
    async def classify(self, message: "Message") -> IntentResult:
        ...
        if len(body) > 5000 or body.count("http") > 5:
            return IntentResult(
                intent=Intent.SPAM, confidence=0.9,
    ...
    def decide(self, intent_result: IntentResult, permission_level: int) -> Action:
        ...
        if intent_result.intent in (Intent.DATA_QUERY, Intent.REQUEST):
            if intent_result.api_name and permission_level >= 2:
                return Action.API_CALL
            return Action.NOTIFY

        if intent_result.intent == Intent.DELEGATION:
